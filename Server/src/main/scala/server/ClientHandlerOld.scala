...
object ClientHandlerOld {
  val maxLoginTries = 3
}

class ClientHandlerOld(var s: Socket) /*extends Runnable with Messageable*/ {
...
//  socket = s
 ...
 /* def interpretCode(message: FileMessage) {
    /**
    ...
    var m: FileMessage = null //declared in case its needed
    ...
    if (!loggedIn) {
      if (code == MessageCode.SEND_AUTH) {
        ...
        if (userid > 0) {
          m = new FileMessage(MessageCode.REQUEST_NAME_AND_OS, String.valueOf(userid))
          username = payload.substring(0, payload.indexOf(" "))
          passwordHash = payload.substring(payload.indexOf(" ") + 1);
        } else {
          loginTries += 1
          if (loginTries >= ClientHandlerOld.maxLoginTries) {
            m = new FileMessage(MessageCode.BLOCK_AUTH, null)
          } else {
            m = new FileMessage(MessageCode.REJECT_AUTH, null)
          }
        }
        ...
        if (Server.validOsTypes.contains(os)) {
         ...
         // fileServer = new FileServer(this)
          messageSender.enqueueMessage(new Message(MessageCode.ACCEPT_AUTH, username + " " + passwordHash));
          insertDevice(deviceName, macaddr, userid)
        } else {
          Server.validOsTypes.foreach(println)
          System.err.println(os)
          payload = message.getCode() + " " + message.getPayload()
          m = new FileMessage(MessageCode.DEVICE_NOT_SUPPORTED, payload)
          messageSender.enqueueMessage(m)
        }
      } else {
        var payload = message.getCode() + " " + message.getPayload()
        m = new FileMessage(MessageCode.NOT_LOGGED_IN, payload)
        messageSender.enqueueMessage(m)
      }
    } else {
      code match {
        case MessageCode.BACKUP_FILE =>
          //database.storeFileInDatabase()
  ...
  override def run() {
    ...
    var m = new FileMessage(MessageCode.REQUEST_AUTH, null)
    messageSender.enqueueMessage(m)
